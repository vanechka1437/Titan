...
// ============================================================================
struct alignas(16) MarketDataEvent {
...
// Fixed-size buffer to collect events during a single matching engine tick.
// Pre-allocated per environment to guarantee zero allocations.
struct EventBuffer {
    ...
    inline void push_update(uint8_t side, Price price, int32_t qty_delta) noexcept {
        if (count < MAX_EVENTS) [[likely]] {
            events[count++] = {MarketDataEvent::Type::LOB_UPDATE, side, 0, price, qty_delta, 0};
        }
    }

    inline void push_trade(uint8_t side, uint16_t owner_id, Price price, OrderQty qty) noexcept {
        if (count < MAX_EVENTS) [[likely]] {
            events[count++] = {
                MarketDataEvent::Type::TRADE,
                side,
                owner_id,
                price,
                -(static_cast<int32_t>(qty)),  // Decrement inventory
...
public:
    ...
    void process_cancel(uint64_t target_order_id, EventBuffer& out_events) noexcept;
